defmodule XparWeb.StatControllerTest do
  ...
  describe "new stat" do
    test "renders form", %{conn: conn} do
      conn = get conn, stat_path(conn, :new)
  ...
  describe "create stat" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, stat_path(conn, :create), stat: @create_attrs

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == stat_path(conn, :show, id)

      conn = get conn, stat_path(conn, :show, id)
  ...
  describe "edit stat" do
    ...
    test "renders form for editing chosen stat", %{conn: conn, stat: stat} do
      conn = get conn, stat_path(conn, :edit, stat)
  ...
  describe "update stat" do
    ...
    test "redirects when data is valid", %{conn: conn, stat: stat} do
      conn = put conn, stat_path(conn, :update, stat), stat: @update_attrs
      assert redirected_to(conn) == stat_path(conn, :show, stat)
  ...
  describe "delete stat" do
    ...
    test "deletes chosen stat", %{conn: conn, stat: stat} do
      ...
      assert redirected_to(conn) == stat_path(conn, :index)
      assert_error_sent 404, fn ->
        get conn, stat_path(conn, :show, stat)
      end
    end
  end
